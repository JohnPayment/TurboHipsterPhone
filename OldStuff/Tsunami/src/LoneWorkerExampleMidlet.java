...
public class LoneWorkerExampleMidlet extends MIDlet implements CommandListener {
    ...
    public void commandAction(Command c, Displayable d) {
        if (c == exit) {
            ...
            new Thread(){
                public void run(){
                    writeToFile();
                     form.append("Write Completed");
                    int result = loneworker.setConfiguration(FILE_NAME);
                     form.append("Set Config Completed");
                    try {
                        FileConnection fc = (FileConnection) Connector.open("file:///Phone/Settings/" + FILE_NAME);
                        fc.delete();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
    ...
    private void writeToFile() {
        ...
        try {
            fc = (FileConnection) Connector.open("file:///Phone/Settings/" + FILE_NAME);
            if(fc.exists())
                fc.delete();
            fc.create();
