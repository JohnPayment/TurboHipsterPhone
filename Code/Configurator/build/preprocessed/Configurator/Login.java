                                  ...
                                  Displayable d)
        {
            ...
            if (c.equals(loginCommand))
            {
                ...
                if (!f.writeTextFile(Path.LOGIN_FILENAME, userInfo.toString()))
                {
                    ...
                    pop = new Popup(m, Strings.getMessage(Messages.SAVING_FAILED),
                        true, m.getLoginScreen());
                    dis.setCurrent(pop);
                }
                else
                {
            ...
            else if (c.equals(readCommand))
            {
                try
                {
                    ...
                    userInfo = new UI(f.getLineAt(0),
                        PasswordEncoder.decode(f.getLineAt(1)),
                        Integer.parseInt(f.getLineAt(2)));
                ...
                catch (IOException ex)
                {
                    ...
                    dis.setCurrent(new Popup(m, ex.getMessage(), true,
                        m.getLoginScreen()));
                }
            }
